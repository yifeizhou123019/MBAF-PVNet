...
class RotEquiAttention(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels=None, angles=(0, 45, 90, 135)):
        ...
        if mid_channels is not None:
            ...
            self.mid_c = max(in_channels // 4, 1)
    ...
    def forward(self, F_in):
        ...
        for ang in self.angles:
            ...
            ], device=device).unsqueeze(0).repeat(B, 1, 1)  # [B,2,3]


            grid = F.affine_grid(theta, F_in.size(), align_corners=True)  # [B,H,W,2]
            F_rot = F.grid_sample(F_in, grid, mode='bilinear', align_corners=True)  # [B,C,H,W]

            # —— 通道注意力分支 —— #
            ca = self.channel_att(F_rot)  # [B, C, 1, 1]
            F_ca = F_rot * ca             # [B, C, H, W]

            # —— 空间注意力分支 —— #
            avg_out = torch.mean(F_ca, dim=1, keepdim=True)   # [B,1,H,W]
            max_out, _ = torch.max(F_ca, dim=1, keepdim=True)  # [B,1,H,W]
            sa_in = torch.cat([avg_out, max_out], dim=1)       # [B,2,H,W]
            sa = self.spatial_att(sa_in)                       # [B,1,H,W]
            F_att = F_ca * sa                                  # [B, C, H, W]
            ...
            F_back = F.grid_sample(F_att, inv_grid, mode='bilinear', align_corners=True)  # [B,C,H,W]

            fused += F_back

        fused = fused / float(len(self.angles))
        return fused
